import type { AppLanguage } from "../../library/types.js"

type Dict = Record<string, string>

const en: Dict = {
  appName: "Doorei",
  welcomeTitle: "Welcome to Doorei",
  welcomeBody:
    "A personal recall library for downloaded video courses. Search and Summary are why it exists; the player is how you watch and leave progress.",
  getStarted: "Get started",
  back: "Back",
  modelsTitle: "Models",
  modelsHint:
    "Persian ASR, English ASR, and the Embedding Model ship with the app. They stay on disk across updates until the model pack changes.",
  modelsMissing: "Required model files are missing from the app bundle.",
  languageTitle: "Language",
  appLanguage: "App language",
  appLanguageHint: "Menus, buttons, and the rest of the UI.",
  courseAsrLanguage: "Spoken language default",
  courseAsrLanguageHint:
    "Default Spoken language for Videos in this Course. Captioning uses the ASR Model for that language.",
  persian: "Persian",
  english: "English",
  requiredModels: "Required models",
  modelPersianAsr: "Persian ASR (Shenava)",
  modelEnglishAsr: "English ASR (Parakeet)",
  modelEmbedding: "Embedding (Search)",
  modelsReady: "Required models are in the app bundle",
  providerOptional: "Provider (optional)",
  providerUrl: "OpenAI-compatible URL",
  providerKey: "Provider key",
  providerModel: "Model id",
  providerModelHintOpenAI:
    "The chat model name this endpoint expects, for example gpt-4o-mini or llama3.1.",
  providerModelHintCursor:
    "Cursor model id, for example composer-2.5. Leave blank for composer-2.5 in fast mode.",
  providerModelHintSdk: "Optional model id for this Provider. Leave blank for its default.",
  providerExtra: "Parameters (JSON)",
  providerExtraHintOpenAI:
    "Optional request fields such as {\"temperature\":0.2,\"max_tokens\":4096,\"reasoning_effort\":\"low\"}.",
  providerExtraHintCursor:
    "Per-model variables such as {\"fast\":true} or thinking/reasoning params from the Cursor catalog.",
  providerNone: "None",
  providerKindOpenAI: "OpenAI-compatible",
  providerKindCodex: "Codex",
  providerKindOpenCode: "OpenCode",
  providerKindCursor: "Cursor",
  providerKindHint: "Improve, Summary, and Ask use this Provider. Search still works without one.",
  providerCursorKey: "Cursor key",
  continue: "Open Library",
  gateHint: "The Library stays closed until language is chosen and every required model file is on disk.",
  newCourse: "New Course",
  courseName: "Course name",
  newSession: "New Session",
  sessionName: "Session name",
  renameSession: "Rename Session",
  deleteSession: "Delete Session",
  deleteSessionWarning:
    "This Session has Videos. Deleting it removes those Videos from the Library, with their Captions, Notes, and Summaries. Files on disk are not deleted.",
  optionalDate: "Date (optional)",
  addVideos: "Add Videos",
  addFolder: "Add folder",
  sessions: "Sessions",
  noVideosInSession: "No videos yet",
  noSessions: "No sessions yet. Create one to start.",
  settings: "Settings",
  search: "Search",
  ask: "Ask",
  summary: "Summary",
  notes: "Notes",
  note: "Note",
  composerPlaceholder: "Write a Note…",
  addAsNote: "Add as Note",
  timestamp: "Timestamp",
  noVideo: "Select a Video to play",
  fileMissing: "File missing",
  relink: "Relink",
  relinkFolder: "Folder moved…",
  deleteVideo: "Delete Video",
  watched: "Watched",
  unwatched: "Unwatched",
  previous: "Previous Video",
  next: "Next Video",
  regenerate: "Regenerate Caption",
  retry: "Retry",
  scopeVideo: "Video",
  scopeSession: "Session",
  scopeCourse: "Course",
  searchPlaceholder: "Search Captions and Notes",
  showAllHits: "Show all ({count})",
  askPlaceholder: "Ask about this Course. Type @ to pin a Session or Video ({chord})",
  askOff: "Ask is off until a Provider is configured.",
  askMention: "Pin a Session or Video",
  askMentionEmpty: "No Session or Video matches.",
  askMentionAdd: "Add a Session or Video to this Ask",
  askMentionRemove: "Remove",
  hitMention: "Pinned",
  you: "You",
  answer: "Answer",
  compactTurn: "Earlier turns (compacted)",
  newConversation: "New Conversation",
  renameConversation: "Rename Conversation",
  deleteConversation: "Delete Conversation",
  conversationTitle: "Title",
  hits: "Hits",
  asking: "Answering…",
  askEmpty: "Ask about this Course. Answers cite Hits — click one to jump to that place in a Video.",
  send: "Send",
  sendHint: "{chord} to send",
  hitThisVideo: "This Video",
  hitThisSession: "This Session",
  hitRestOfCourse: "Course",
  noSummary: "No Summary yet. Configure a Provider to generate one.",
  noSummaryYet: "No Summary yet.",
  noCaptionForSummary: "Caption this Video first.",
  generateSummary: "Generate Summary",
  generateMissingSummaries: "Generate missing Summaries",
  summaryGenerating: "Writing Summary…",
  summaryImproving: "Improving Caption, then writing Summary…",
  noNotes: "No Notes on this Video.",
  captions: "Caption",
  noCaptions: "No Caption on this Video.",
  outputLanguage: "Output language",
  outputLanguageHint: "Language of Summary and Ask for this Course.",
  spokenDefault: "Default Spoken language",
  autoplay: "Autoplay next Video",
  confetti: "Confetti on Watched",
  subtitles: "Subtitles",
  speed: "Speed",
  play: "Play",
  pause: "Pause",
  mute: "Mute",
  unmute: "Unmute",
  volume: "Volume",
  fullscreen: "Full screen",
  exitFullscreen: "Exit full screen",
  seek: "Seek",
  seekBack: "Back 5 seconds",
  seekForward: "Forward 5 seconds",
  captionStyle: "Caption style",
  captionColor: "Text color",
  captionBackground: "Background",
  showCaptions: "Show Captions",
  playError: "This Video could not be played",
  improvePrompt: "Improve prompt",
  summaryPrompt: "Summary prompt",
  askPrompt: "Ask prompt",
  askContextBudget: "Ask context budget (tokens)",
  save: "Save",
  cancel: "Cancel",
  providerKind: "Provider kind",
  jobs: "Jobs",
  clearFailedJobs: "Clear failed Jobs",
  emptyLibrary: "Create a Course to start.",
  create: "Create",
  editCourse: "Edit Course",
  courseNameTooShort: "Enter a Course name.",
  courseNameTooLong: "Keep the Course name under 120 characters.",
  promptTooShort: "Use at least 14 characters. A shorter prompt is not meaningful.",
  promptTooLong: "Keep this prompt under 8000 characters.",
  editNote: "Edit Note",
  markWatchedAtEnd: "Mark Watched when a Video ends",
  fromFolder: "Original folder prefix",
  spokenLanguage: "Spoken language",
  providerOn: "Provider",
  providerOff: "No Provider",
  settingsPlayer: "Player",
  settingsLanguage: "Language",
  settingsProvider: "Provider",
  settingsApp: "App",
  appVersion: "Version",
  updateIdle: "No update check yet.",
  updateDisabled: "Updates are checked in the packaged app.",
  updateChecking: "Checking for updates…",
  updateCurrent: "Doorei is up to date.",
  updateAvailable: "Version {version} is available.",
  updateDownloading: "Downloading update {percent}%",
  updateReady: "Restart to install {version}. Model files already on disk are not downloaded again.",
  updateRestart: "Restart to update",
  updateCheck: "Check for updates",
  updateError: "Update failed",
  updateHint:
    "In-app updates download the app only. The ASR and Embedding Model pack stays in user data until the pack version changes.",
  hideLibrary: "Hide Library",
  showLibrary: "Show Library",
  hideToolPane: "Hide tools",
  showToolPane: "Show tools",
  switchCourse: "Switch Course",
  commandPlaceholder: "Search Courses and actions…",
  commandEmpty: "No results.",
  commandCourses: "Courses",
  commandActions: "Actions",
  toggleNote: "Toggle Note",
  toggleLibrary: "Toggle Library",
  toggleToolPane: "Toggle Tools",
  moreActions: "More actions",
  transparent: "Transparent",
  shortcuts: "Keyboard shortcuts",
  shortcutsIntro: "Keys depend on where focus is.",
  shortcutSectionApp: "App",
  shortcutSectionAppWhen: "Anywhere in the Library.",
  shortcutSectionPlayer: "Player",
  shortcutSectionPlayerWhen:
    "When a Video is selected, and focus is not in a field, checkbox, or a control outside the player.",
  shortcutSectionTyping: "Search, Ask, and Notes",
  shortcutSectionTypingWhen: "When typing in that field.",
  shortcutOr: "or",
  shortcutSend: "Send Ask or save a Note",
  playPause: "Play or pause",
  speedUp: "Faster",
  speedDown: "Slower",
  volumeUp: "Volume up",
  volumeDown: "Volume down",
  keySpace: "Space",
  keyEnter: "Enter",
  keyReturn: "Return",
  keyShift: "Shift"
}

const fa: Dict = {
  appName: "دوری",
  welcomeTitle: "به دوری خوش آمدید",
  welcomeBody:
    "کتابخانهٔ مرور شخصی برای دوره‌های ویدیویی دانلودشده. جستجو و خلاصه دلیل وجود آن است؛ پخش‌کننده راه تماشا و به‌جاگذاشتن پیشرفت است.",
  getStarted: "شروع کنید",
  back: "بازگشت",
  modelsTitle: "مدل‌ها",
  modelsHint:
    "بازشناسی فارسی، بازشناسی انگلیسی و مدل جاسازی با برنامه می‌آیند. تا وقتی بستهٔ مدل عوض نشود، با به‌روزرسانی دوباره دانلود نمی‌شوند.",
  modelsMissing: "فایل‌های مدل لازم در بستهٔ برنامه نیستند.",
  languageTitle: "زبان",
  appLanguage: "زبان برنامه",
  appLanguageHint: "منوها، دکمه‌ها و بقیهٔ رابط برنامه.",
  courseAsrLanguage: "زبان گفتار پیش‌فرض",
  courseAsrLanguageHint:
    "زبان گفتار پیش‌فرض ویدیوهای این دوره. کپشن‌سازی از مدل بازشناسی همان زبان استفاده می‌کند.",
  persian: "فارسی",
  english: "انگلیسی",
  requiredModels: "مدل‌های لازم",
  modelPersianAsr: "بازشناسی فارسی (شنوا)",
  modelEnglishAsr: "بازشناسی انگلیسی (پاراکیت)",
  modelEmbedding: "مدل جاسازی (جستجو)",
  modelsReady: "مدل‌های لازم در بستهٔ برنامه هستند",
  providerOptional: "ارائه‌دهنده (اختیاری)",
  providerUrl: "نشانی سازگار با OpenAI",
  providerKey: "کلید ارائه‌دهنده",
  providerModel: "شناسه مدل",
  providerModelHintOpenAI:
    "نام مدلی که این نشانی انتظار دارد، مثلاً gpt-4o-mini یا llama3.1.",
  providerModelHintCursor:
    "شناسه مدل کورسر، مثلاً composer-2.5. خالی بگذارید تا composer-2.5 در حالت سریع استفاده شود.",
  providerModelHintSdk: "شناسه مدل اختیاری برای این ارائه‌دهنده. خالی یعنی پیش‌فرض خودش.",
  providerExtra: "پارامترها (JSON)",
  providerExtraHintOpenAI:
    "فیلدهای اختیاری درخواست مثل {\"temperature\":0.2,\"max_tokens\":4096,\"reasoning_effort\":\"low\"}.",
  providerExtraHintCursor:
    "متغیرهای مدل مثل {\"fast\":true} یا پارامترهای thinking از فهرست کورسر.",
  providerNone: "هیچ",
  providerKindOpenAI: "سازگار با OpenAI",
  providerKindCodex: "Codex",
  providerKindOpenCode: "OpenCode",
  providerKindCursor: "Cursor",
  providerKindHint: "بهبود، خلاصه و پرسش از این ارائه‌دهنده استفاده می‌کنند. جستجو بدون آن هم کار می‌کند.",
  providerCursorKey: "کلید کورسر",
  continue: "ورود به کتابخانه",
  gateHint: "کتابخانه تا انتخاب زبان و بودن همهٔ فایل‌های مدل لازم روی دیسک باز نمی‌شود.",
  newCourse: "دورهٔ تازه",
  courseName: "نام دوره",
  newSession: "نشست تازه",
  sessionName: "نام نشست",
  renameSession: "تغییر نام نشست",
  deleteSession: "حذف نشست",
  deleteSessionWarning:
    "این نشست ویدیو دارد. حذف آن، آن ویدیوها را با کپشن، یادداشت و خلاصه‌شان از کتابخانه برمی‌دارد. فایل‌ها روی دیسک حذف نمی‌شوند.",
  optionalDate: "تاریخ (اختیاری)",
  addVideos: "افزودن ویدیو",
  addFolder: "افزودن پوشه",
  sessions: "نشست‌ها",
  noVideosInSession: "هنوز ویدیویی نیست",
  noSessions: "هنوز نشستی نیست. یکی بسازید.",
  settings: "تنظیمات",
  search: "جستجو",
  ask: "پرسش",
  summary: "خلاصه",
  notes: "یادداشت‌ها",
  note: "یادداشت",
  composerPlaceholder: "یک یادداشت بنویسید…",
  addAsNote: "افزودن یادداشت",
  timestamp: "زمان",
  noVideo: "یک ویدیو را برای پخش انتخاب کنید",
  fileMissing: "فایل پیدا نشد",
  relink: "اتصال دوباره",
  relinkFolder: "پوشه جابه‌جا شده…",
  deleteVideo: "حذف ویدیو",
  watched: "دیده‌شده",
  unwatched: "نادیده‌شده",
  previous: "ویدیوی قبلی",
  next: "ویدیوی بعدی",
  regenerate: "تولید دوبارهٔ کپشن",
  retry: "تلاش دوباره",
  scopeVideo: "ویدیو",
  scopeSession: "نشست",
  scopeCourse: "دوره",
  searchPlaceholder: "جستجو در کپشن و یادداشت",
  showAllHits: "نمایش همه ({count})",
  askPlaceholder: "از این دوره بپرسید. با @ یک نشست یا ویدیو سنجاق کنید ({chord})",
  askOff: "پرسش تا پیکربندی ارائه‌دهنده خاموش است.",
  askMention: "سنجاق کردن نشست یا ویدیو",
  askMentionEmpty: "نشست یا ویدیویی هم‌خوان نیست.",
  askMentionAdd: "افزودن نشست یا ویدیو به این پرسش",
  askMentionRemove: "برداشتن",
  hitMention: "سنجاق‌شده",
  you: "شما",
  answer: "پاسخ",
  compactTurn: "فشردهٔ نوبت‌های قبلی",
  newConversation: "گفتگوی تازه",
  renameConversation: "تغییر نام گفتگو",
  deleteConversation: "حذف گفتگو",
  conversationTitle: "عنوان",
  hits: "ارجاع‌ها",
  asking: "در حال پاسخ…",
  askEmpty: "از این دوره بپرسید. پاسخ‌ها به ارجاع‌ها استناد می‌کنند — روی هر کدام بزنید تا به همان جا در ویدیو بروید.",
  send: "ارسال",
  sendHint: "ارسال با {chord}",
  hitThisVideo: "این ویدیو",
  hitThisSession: "این نشست",
  hitRestOfCourse: "دوره",
  noSummary: "خلاصه‌ای نیست. برای تولید، ارائه‌دهنده را تنظیم کنید.",
  noSummaryYet: "خلاصه‌ای نیست.",
  noCaptionForSummary: "اول برای این ویدیو کپشن بسازید.",
  generateSummary: "تولید خلاصه",
  generateMissingSummaries: "تولید خلاصه‌های مانده",
  summaryGenerating: "در حال نوشتن خلاصه…",
  summaryImproving: "در حال بهبود کپشن و نوشتن خلاصه…",
  noNotes: "یادداشتی روی این ویدیو نیست.",
  captions: "کپشن",
  noCaptions: "کپشنی روی این ویدیو نیست.",
  outputLanguage: "زبان خروجی",
  outputLanguageHint: "زبان خلاصه و پرسش برای این دوره.",
  spokenDefault: "زبان گفتار پیش‌فرض",
  autoplay: "پخش خودکار ویدیوی بعدی",
  confetti: "شادی‌پاش هنگام دیده‌شده",
  subtitles: "زیرنویس",
  speed: "سرعت",
  play: "پخش",
  pause: "مکث",
  mute: "بی‌صدا",
  unmute: "با صدا",
  volume: "صدا",
  fullscreen: "تمام‌صفحه",
  exitFullscreen: "خروج از تمام‌صفحه",
  seek: "جابه‌جایی",
  seekBack: "۵ ثانیه به عقب",
  seekForward: "۵ ثانیه به جلو",
  captionStyle: "سبک کپشن",
  captionColor: "رنگ متن",
  captionBackground: "پس‌زمینه",
  showCaptions: "نمایش کپشن",
  playError: "این ویدیو پخش نشد",
  improvePrompt: "دستور بهبود",
  summaryPrompt: "دستور خلاصه",
  askPrompt: "دستور پرسش",
  askContextBudget: "بودجهٔ بافت پرسش (توکن)",
  save: "ذخیره",
  cancel: "لغو",
  providerKind: "نوع ارائه‌دهنده",
  jobs: "کارها",
  clearFailedJobs: "پاک کردن کارهای ناموفق",
  emptyLibrary: "برای شروع یک دوره بسازید.",
  create: "بساز",
  editCourse: "ویرایش دوره",
  courseNameTooShort: "نام دوره را وارد کنید.",
  courseNameTooLong: "نام دوره را کوتاه‌تر از ۱۲۰ نویسه نگه دارید.",
  promptTooShort: "دست‌کم ۱۴ نویسه بنویسید. دستور کوتاه‌تر بی‌معناست.",
  promptTooLong: "این دستور را کوتاه‌تر از ۸۰۰۰ نویسه نگه دارید.",
  editNote: "ویرایش یادداشت",
  markWatchedAtEnd: "علامت دیده‌شده در پایان ویدیو",
  fromFolder: "پیشوند پوشهٔ قبلی",
  spokenLanguage: "زبان گفتار",
  providerOn: "ارائه‌دهنده",
  providerOff: "بدون ارائه‌دهنده",
  settingsPlayer: "پخش",
  settingsLanguage: "زبان",
  settingsProvider: "ارائه‌دهنده",
  settingsApp: "برنامه",
  appVersion: "نسخه",
  updateIdle: "هنوز بررسی به‌روزرسانی نشده است.",
  updateDisabled: "به‌روزرسانی در نسخهٔ بسته‌بندی‌شده بررسی می‌شود.",
  updateChecking: "در حال بررسی به‌روزرسانی…",
  updateCurrent: "دوری به‌روز است.",
  updateAvailable: "نسخهٔ {version} آماده است.",
  updateDownloading: "دانلود به‌روزرسانی {percent}٪",
  updateReady: "برای نصب {version} دوباره باز کنید. فایل‌های مدل روی دیسک دوباره دانلود نمی‌شوند.",
  updateRestart: "بازگشایی و به‌روزرسانی",
  updateCheck: "بررسی به‌روزرسانی",
  updateError: "به‌روزرسانی ناموفق بود",
  updateHint:
    "به‌روزرسانی درون‌برنامه فقط خود برنامه را دانلود می‌کند. بستهٔ مدل بازشناسی و جاسازی تا تغییر نسخهٔ بسته در داده‌های کاربر می‌ماند.",
  hideLibrary: "پنهان کردن کتابخانه",
  showLibrary: "نمایش کتابخانه",
  hideToolPane: "پنهان کردن ابزارها",
  showToolPane: "نمایش ابزارها",
  switchCourse: "تغییر دوره",
  commandPlaceholder: "جستجوی دوره و دستور…",
  commandEmpty: "نتیجه‌ای نیست.",
  commandCourses: "دوره‌ها",
  commandActions: "دستورها",
  toggleNote: "نمایش یا پنهان کردن یادداشت",
  toggleLibrary: "نمایش یا پنهان کردن کتابخانه",
  toggleToolPane: "نمایش یا پنهان کردن ابزارها",
  moreActions: "دستورهای بیشتر",
  transparent: "شفاف",
  shortcuts: "میانبرهای صفحه‌کلید",
  shortcutsIntro: "کلیدها به محل تمرکز بستگی دارند.",
  shortcutSectionApp: "برنامه",
  shortcutSectionAppWhen: "همه‌جا در کتابخانه.",
  shortcutSectionPlayer: "پخش",
  shortcutSectionPlayerWhen:
    "وقتی ویدیویی انتخاب شده و تمرکز روی فیلد، چک‌باکس، یا کنترل بیرون پخش‌کننده نیست.",
  shortcutSectionTyping: "جستجو، پرسش و یادداشت",
  shortcutSectionTypingWhen: "وقتی در همان فیلد می‌نویسید.",
  shortcutOr: "یا",
  shortcutSend: "ارسال پرسش یا ذخیرهٔ یادداشت",
  playPause: "پخش یا مکث",
  speedUp: "سرعت بیشتر",
  speedDown: "سرعت کمتر",
  volumeUp: "صدای بیشتر",
  volumeDown: "صدای کمتر",
  keySpace: "فاصله",
  keyEnter: "Enter",
  keyReturn: "Return",
  keyShift: "Shift"
}

export function t(language: AppLanguage | null, key: keyof typeof en): string {
  const dict = (language ?? "fa") === "fa" ? fa : en
  return dict[key] ?? en[key]
}

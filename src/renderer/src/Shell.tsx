...
export function Shell({ snapshot }: Props) {
  ...
  const selected = snapshot.videos.find(
  ...
  useEffect(() => {
    if (
      playAfterSelectId.current &&
      playAfterSelectId.current !== selected?.id
    ) {
  ...
  async function selectAndPlay(
    method: "nextVideoId" | "previousVideoId"
  ): Promise<boolean> {
  ...
  async function addVideos(
    ...
    const sessionId =
      targetSessionId ??
      selected?.sessionId ??
      courseSessions[courseSessions.length - 1]?.id;
    if (!sessionId) return;
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col">
                  ...
                  <Player
                    ...
                    onEnded={() => {
                      ...
                      if (snapshot.settings.autoplay)
                        void selectAndPlay("nextVideoId");
                    }}
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            if (toDir)
                              setPrompt({ kind: "from-folder", toDir });
                          }}
                        >
                          {t(lang, "relinkFolder")}
                        </Button>
                  ...
                  onSubmit={(event) => {
                    ...
                    const timestampSeconds = stampOn
                      ? videoRef.current?.currentTime ??
                        selected.playbackPositionSeconds
                      : null;
      ...
      <PromptDialog
        ...
        onSubmit={(text) => {
          if (prompt?.kind === "note")
            void window.doorei.call("editNote", prompt.id, text);
        }}
      />
      <PromptDialog
...
function jobStatusLine(snapshot: LibrarySnapshot, jobs: Job[]): string {
  return jobs
    .map((job) => {
      const name = snapshot.videos.find(
        (video) => video.id === job.videoId
      )?.name;

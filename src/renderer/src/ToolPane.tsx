      ...
      >
        ...
        {activity === "captions" ? (
          <CaptionList
            lang={lang}
            segments={
              (snapshot.improvedCaption ?? snapshot.caption)?.segments ?? []
            }
        ...
        {activity === "summary" ? (
          <SummaryPane snapshot={snapshot} lang={lang} />
        ) : null}
        {activity === "notes" ? (
    ...
    <>
      <div className="flex gap-2">
        ...
        <Select
          ...
          items={scopeItems}
          onValueChange={(value) => {
            if (value === "video" || value === "session" || value === "course")
              setScope(value);
          }}
        >
...
function SummaryPane({
  ...
  const hasCaption = Boolean(
    (snapshot.improvedCaption ?? snapshot.caption)?.segments.length
  );
...
function CaptionList({
  ...
  if (segments.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">{t(lang, "noCaptions")}</p>
    );
  }
        ...
        {segments.map((segment, index) => {
          ...
          return (
            <li key={`${segment.startSeconds}-${index}`}>
              ...
              >
                <span
                  className="w-10 shrink-0 pt-0.5 font-medium text-white/45 tabular-nums"
                  dir="ltr"
                >

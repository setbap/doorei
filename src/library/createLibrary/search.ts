...
import { dot, l2Normalize } from "./helpers.js"
...
export async function collectHits(
  ...
  const neededCourses = new Set<string>()
  for (const video of videos) {
    const courseId = courseIdOfVideo(state, video.id)
    if (courseId) neededCourses.add(courseId)
  }
  for (const courseId of neededCourses) core.loadEmbeddingsForCourse(courseId)
  ...
  const [rawQuery] = await deps.embedder.embed([needle])
  if (!rawQuery || rawQuery.every((value) => value === 0)) return hits
  const queryVector = l2Normalize(rawQuery)
  for (const video of videos) {
...
export function lexicalSearch(
  ...
  const notesByVideo = new Map<string, typeof state.notes>()
  for (const note of state.notes) {
    const list = notesByVideo.get(note.videoId)
    if (list) list.push(note)
    else notesByVideo.set(note.videoId, [note])
  }
  const hits: Hit[] = []
  for (const video of videosInScope(state, input.scope, input.videoIds)) {
    ...
    for (const note of notesByVideo.get(video.id) ?? []) {
